...
class HotPotQATask(Task):
    ...
    @staticmethod
    def generate_self_reflection(z, question, gpt, tokenizer_llm, generation_config):
        reflection_mapping = []
        trajectories = ""
        ...
        for traj in failed_trajectories:
            ...
            inputs = tokenizer_llm(reflect_prompt, return_tensors="pt").to("cuda")
            outputs = gpt.generate(**inputs, generation_config=generation_config)
            reflection = tokenizer_llm.batch_decode(outputs, skip_special_tokens=True)[0][len(reflect_prompt):].strip()
            print(reflection)
            print('>'*30)
            trajectories += "Reflection: " + reflection + "\n"
